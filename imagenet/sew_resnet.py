...
def cal_batch_both_mi_mask_dividet0(data):
    device = data.device
    t, b, c, h, w = data.size()
    mi_matrix = torch.zeros(t, h*w).to(device)
    data = data.view(t, b*c, -1).int()
    sample = data 
    sample_t1 = sample[0, :, :]
    ...
    for ti in range(0, t):
        ...
        global_mi_value = mutual_info_score(sample_t1_1d, sample_t2_1d)
        for elei in range(h*w):
            ele_t1 = sample_t1[:, elei]
            ele_t2 = sample_t2[:, elei]
            local_mi_value = mutual_info_score(ele_t1, ele_t2)
            mi_matrix[ti, elei] = local_mi_value * global_mi_value
    mi_matrix[:, :] = mi_matrix[:, :] / mi_matrix[0, :]
    mi_matrix[0, :] = 0
    mi_matrix = mi_matrix.view(t, h, w)
    return mi_matrix

class TModule_afterneu(nn.Module):
    def __init__(self) -> None:
        super(TModule_afterneu, self).__init__()
        self.epochs = 320
    ...
    def forward(self, x: torch.Tensor, epoch):
        ...
        # mi mask 
        mask_prob_mi = cal_batch_both_mi_mask_dividet0(x) 
        mask_prob_mi = mask_prob_mi * pro_epoch 
        random_tensor = torch.rand_like(x)
        mask = random_tensor < mask_prob_mi.unsqueeze(2).unsqueeze(3).unsqueeze(4)
        x[mask] = 0
        return x
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.train_firing_num_tlist = torch.zeros(self.T)
        self.train_firing_rate = 0
        self.test_firing_num_tlist = torch.zeros(self.T)
        self.test_firing_rate = 0
...
class SEWResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.tm = TModule_afterneu()

        self.avgpool = layer.AdaptiveAvgPool2d((1, 1))
        self.fc = layer.Linear(512 * block.expansion, num_classes)

        self._firing_num_t = torch.zeros(self.T)
        self._all_elements_num = 0 # for firing rate calculation 

        for m in self.modules():
            if isinstance(m, layer.Conv2d):
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
            elif isinstance(m, IFNode5):
                # register hook
                m.register_forward_hook(self.spike_count)
    ...
    def spike_count(self, module, input, output):
        with torch.no_grad():
            self._firing_num_t = self._firing_num_t.to(output.device)
            self._firing_num_t += torch.sum(output, dim=(1, 2, 3, 4))
            self._all_elements_num += output.numel()
    
    def reset_spike_count(self):
        self._firing_num_t = torch.zeros(self.T)
        self._all_elements_num = 0 
    ...
    def _forward_impl(self, x, epoch):
        ...
        x = self.fc(x)
        firing_rate = self._firing_num_t.sum() / self._all_elements_num

        return x, self._firing_num_t, firing_rate
