...
def train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, print_freq, scaler=None):
    ...
    for image, target in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if scaler is not None:
            with torch.amp.autocast('cuda'):
                output, firing_num_t, firing_rate = model(image, epoch)

                # calculate loss using weight based on firing number 
                firing_num = firing_num_t.sum().item()
                firing_num_tw = firing_num_t / firing_num 
                output = (output * firing_num_tw.unsqueeze(1).unsqueeze(2)).sum(0)
        ...
        model.module.reset_spike_count()
        if output.dim() == 3:
            with torch.no_grad():
    ...
    print(f'train a epoch time: {time.time() - start_time}')
...
def evaluate(model, criterion, data_loader, device, print_freq=100, header='Test:'):
    ...
    with torch.no_grad():
        for image, target in metric_logger.log_every(data_loader, print_freq, header):
            ...
            output, firing_num_t, firing_rate = model(image)
            loss = criterion(output, target)
            ...
            if output.dim() == 3:
                with torch.no_grad():
                    # output = output.mean(0)
                    output = output[0] # only use the first time step output 
...
def main(args):
    ...
    if args.model in sew_resnet.__dict__:
        model = sew_resnet.__dict__[args.model](pretrained=False, cnf='ADD', spiking_neuron=sew_resnet.IFNode5,
...
def parse_args():
    ...
    parser.add_argument('--cos_lr_T', default=-1, type=int, help='T_max of CosineAnnealingLR.')
    parser.add_argument('--load', type=str, default=None, help='the pt file path for loading pre-trained ANN weights')
    parser.add_argument('--finetune', type=str, default=None, help='finetune from loaded pt file')
